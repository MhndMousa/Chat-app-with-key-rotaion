...
def encrypt():
     ...
     for letter in range(0,len(word)):
         ...
         if key + toNum(word[letter]) > 26:
                 temp = abs(toNum(word[letter]) +(key) - 26)
                 word[letter] = toChar( int(temp))
         elif toNum(word[letter]) > 0 and toNum(word[letter]) <= 26:
             word[letter] = toChar(toNum(word[letter]) + int(key))
         else:
             continue
     words = "".join(word)
     #data = myarray.read(1024)
     data = words
     userNumber['text'] = data#e1.get()
     s2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
     s2.connect(("localhost",8180))
     #while (data):
     s2.send(words)
         #l = myarray.read(1024)
     s2.close()
